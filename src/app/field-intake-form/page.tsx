...
export default function IntakeFormPage() {
  ...
  function generatePDF() {
    ...
    const checkPage = (needed: number) => {
      if (y + needed > bodyBottom) { doc.addPage(); y = 18; }
    };

    const field = (label: string, value?: string) => {
      const val = value?.trim() || '-';
      const lines = doc.splitTextToSize(val, contentW);
      checkPage(10 + lines.length * 5);
    ...
    const fieldRow = (left: [string, string?], right?: [string, string?]) => {
      ...
      const leftVal = left[1]?.trim() || '-';
      const leftLines = doc.splitTextToSize(leftVal, colW);
      const rightVal = right ? (right[1]?.trim() || '-') : [];
      const rightLines = right ? doc.splitTextToSize(rightVal, colW) : [];
      const maxLines = Math.max(leftLines.length, rightLines.length);
      checkPage(10 + maxLines * 5 + 6);
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8);
      doc.setTextColor(13, 36, 75);
      doc.text(left[0].toUpperCase(), margin, y);
      if (right) doc.text(right[0].toUpperCase(), margin + colW + 8, y);
      y += 5;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      doc.setTextColor(58, 58, 58);
      doc.text(leftLines, margin, y);
      if (right && rightLines.length) doc.text(rightLines, margin + colW + 8, y);
      y += maxLines * 5 + 6;
    };
    ...
    const header = (num: string, title: string) => {
      ...
      doc.text(`SECTION ${num}`, margin + 4, y + 5.5);
      doc.setFontSize(11);
      doc.setTextColor(255, 255, 255);
      doc.text(title, margin + 4, y + 11.5);
      y += 20;
    };
    ...
    const addFooters = () => {
      ...
      for (let i = 1; i <= total; i++) {
        ...
        doc.line(margin, pageH - footerH - 1, pageW - margin, pageH - footerH - 1);
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(7.5);
        doc.setTextColor(90, 100, 120);
        doc.text('IP HOUSE  ·  CONFIDENTIAL & PROPRIETARY', margin, pageH - footerH + 2.5);
        doc.text(`Page ${i}`, pageW - margin, pageH - footerH + 2.5, { align: 'right' });
        doc.text('Special Investigations Unit  ·  Online Investigations Team', margin, pageH - footerH + 7);
      }
    };
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8);
    doc.text('INTAKE  >  SCOPING  >  ESTIMATE  >  KICKOFF', margin, 13);
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(22);
    doc.text('Online Investigation Intake', margin, 27);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.text('Special Investigations Unit  ·  Online Investigations Team', margin, 36);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.setTextColor(255, 255, 255);
    doc.text('IP HOUSE', pageW - margin, 13, { align: 'right' });
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    const dateStr = now.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
    const timeStr = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });
    doc.text(dateStr, pageW - margin, 21, { align: 'right' });
    doc.text(timeStr, pageW - margin, 28, { align: 'right' });

    y = 58;

    // ── Email notice box ─────────────────────────────────────────────
    doc.setDrawColor(13, 36, 75);
    doc.setLineWidth(0.4);
    doc.rect(margin, y, contentW, 16, 'D');
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8);
    doc.setTextColor(13, 36, 75);
    doc.text('PLEASE EMAIL THIS PDF (WITH ANY SUPPORTING ATTACHMENTS) TO', margin + 4, y + 6);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9.5);
    doc.setTextColor(58, 58, 58);
    doc.text('osint@ip-house.com', margin + 4, y + 12.5);
    y += 24;

    // ── Section 01 ───────────────────────────────────────────────────
    header('01', 'Request Details');
    fieldRow(['Full Name', form.req_name], ['Department / Team', form.req_department]);
    fieldRow(['Client', form.client_name], ['Client Status', form.client_status]);
    ...
    if (ratesVisible) {
      fieldRow(['Rate Type', form.rate_type], ['Agreed Amount', `${form.rate_currency} ${form.rate_amount}`]);
      if (form.rate_notes) field('Rate Notes', form.rate_notes);
    }

    // ── Section 02 ───────────────────────────────────────────────────
    header('02', 'Investigation Overview');
    field('Objectives', form.obj);
    if (form.deliverables) field('Expected Deliverables', form.deliverables);
    if (form.intended_use.length) field('Intended Use of Findings', form.intended_use.join(', '));
    ...
    if (form.scale) field('Suspected Scale', form.scale);
    fieldRow(['Written Status Updates', form.status_updates], ['Meetings or Calls With Client', form.meetings_calls]);
    if (form.engagement_notes) field('Engagement Notes', form.engagement_notes);
    fieldRow(['Proposal / Estimate Required By', formatLongDate(form.deadline_proposal) || 'Not specified'], ['Investigation Deadline', formatLongDate(form.deadline_investigation) || 'Not specified']);
    ...
    field('Identifiers', ids.length ? ids.join('\n') : '—');
    if (form.geo_info) field('Geographic Information', form.geo_info);
    if (form.additional_ids) field('Additional Target Information', form.additional_ids);

    // ── Section 04 ───────────────────────────────────────────────────
    header('04', 'Prior Investigation History');
    field('Has an Investigation into This Matter Previously Been Attempted?', form.prior_inv);
